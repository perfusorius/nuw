    ...
    if (!ver || ver === 'latest' || ver === 'stable') {
        ...
        getCurrentVersion(function (err, current) {
            ...
            fn(function (err, latest) {
                ...
                if (semver.lt(latest, '0.7.11')) {
                    return cb(null, 'Node versions before 0.7.11 are not supported by this tool');
                }
                if (semver.lt(latest, current.ver)) {
                    return cb(null, 'If you wish to install an older version you have to uninstall the current version first');
                }
                else if (semver.eq(latest, current.ver)) {
                    return cb(null, 'The latest version "' + latest + '" is already installed');
                }
                download(latest, function (err, file) {
                    if (err) {
                        return cb(err);
                    }
                    install(file);
        ...
        getCurrentVersion(function (err, current) {
            ...
            if (semver.lt(ver, current.ver)) {
                return cb(null, 'If you wish to install an older version you have to uninstall the current version first');
            }
            else if (semver.eq(ver, current.ver)) {
                return cb(null, 'Version "' + ver + '" is already installed');
            }
